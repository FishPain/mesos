document.addEventListener("DOMContentLoaded", function () {
  ...
  function updatePositions() {
    ...
    var block1Bounds = document
      .getElementById("block1")
      .getBoundingClientRect();
    var block2Bounds = document
      .getElementById("block2")
      .getBoundingClientRect();
    var spaceBetween = block2Bounds.left - block1Bounds.right;
    var centerY =
      window.innerHeight / 2 -
      document.querySelector(".draggable").offsetHeight / 2;
    var startX =
      block1Bounds.right +
      spaceBetween / 2 -
      document.querySelector(".draggable").offsetWidth / 2;
  ...
  function updateRelativePositions() {
    ...
    block3.style.left = `${
      overlayBounds.right + 5 * (window.innerWidth / 100)
    }px`;
    ...
    block5.style.top = `${
      overlayBounds.top + 15 * (window.innerWidth / 100)
    }px`;

    // 5vw to the left of the overlay for block4 and block6, considering its own width
    block4.style.left = `${
      overlayBounds.left - block4.offsetWidth - 5 * (window.innerWidth / 100)
    }px`;
    block4.style.top = `${
      overlayBounds.top - block4.offsetWidth - 15 * (window.innerWidth / 100)
    }px`;

    block6.style.left = `${
      overlayBounds.left - block6.offsetWidth - 5 * (window.innerWidth / 100)
    }px`;
    block6.style.top = `${
      overlayBounds.top - block6.offsetWidth - 5 * (window.innerWidth / 100)
    }px`;
  }
  ...
  function logLaneAndDistance(newX) {
    ...
    var block2Bounds = document
      .getElementById("block2")
      .getBoundingClientRect();
    var overlayBounds = document
      .getElementById("overlay")
      .getBoundingClientRect();
    var block4Bounds = document
      .getElementById("block4")
      .getBoundingClientRect();
    var spaceBetween = block2Bounds.left - block1Bounds.right;
  ...
  function roundToNearest50(num, freq) {
    return Math.round(num / 100) * freq;
  }

  function updateERP(lane, distance) {
    distance = roundToNearest50(distance, 50);
